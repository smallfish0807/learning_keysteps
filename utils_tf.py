...
class MyDQNAgent(dqn_agent.DQNAgent):
    ...
    def __init__(self, sess, num_actions, summary_writer=None):
        super(MyDQNAgent, self).__init__(sess,
                                         num_actions,
...
class MyIQNAgent(implicit_quantile_agent.ImplicitQuantileAgent):
    """Sample IQN agent to visualize Q-values and rewards."""
    def __init__(self, sess, num_actions, summary_writer=None):
        super(MyIQNAgent, self).__init__(sess,
                                         num_actions,
                                         summary_writer=summary_writer)
        self.q_values = [[] for _ in range(num_actions)]
        self.rewards = []
    ...
    def _select_action(self):
        action = super(MyIQNAgent, self)._select_action()
        q_vals = self._sess.run(self._net_outputs.q_values,
                                {self.state_ph: self.state})[0]
        for i in range(len(q_vals)):
            self.q_values[i].append(q_vals[i])
...
class MyRainbowAgent(rainbow_agent.RainbowAgent):
    ...
    def __init__(self, sess, num_actions, summary_writer=None):
        super(MyRainbowAgent, self).__init__(sess,
                                             num_actions,
                                             summary_writer=summary_writer)
        self.rewards = []
...
class MyRunner(run_experiment.Runner):
    ...
    def run_eval_episodes(self, num_episodes):
        ...
        while episode_count < num_episodes:
            ...
            while True:
                ...
                if is_terminal:
                    print('Terminal')
                    self._agent.rewards.append(reward)
                    action = self._agent.begin_episode(observation)
                else:
                    action = self._agent.step(reward, observation)
            episode_count += 1
            print(episode_count, np.sum(self._agent.rewards))
            self._agent.rewards = []
...
def create_runner(base_dir, trained_agent_ckpt_path, agent='dqn'):
    if agent == 'dqn':
        ...
        raise ValueError(
            'Currently not supported, since iqn_network object has no attribute q_values'
        )
    else:
...
def run(agent, game, num_steps, root_dir, restore_ckpt):
    ...
    runner.run_eval_episodes(num_episodes=2)
    #runner.visualize(os.path.join(base_dir, 'images'), num_global_steps=num_steps)


def load_model_tf(agent, game, restore_ckpt, root_dir='result_tf'):
    config = """
    atari_lib.create_atari_environment.game_name = '{}'
    atari_lib.create_atari_environment.sticky_actions = True
    WrappedReplayBuffer.replay_capacity = 300
    dqn_agent.DQNAgent.tf_device = '/gpu:0'
    """.format(game)
    base_dir = os.path.join(root_dir, 'agent_viz', game, agent)
    gin.parse_config(config)
    with tf.Graph().as_default():
        runner = create_runner(base_dir, restore_ckpt, agent)
        print('Loading {}'.format(restore_ckpt))
        for v in tf.trainable_variables():
            print("{:15} {:15}".format(v.name, str(v.get_shape())))
    return runner._agent
